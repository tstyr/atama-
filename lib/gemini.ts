...
【数式の書き方】
- 分数: \\frac{分子}{分母} 例: \\frac{1}{2}
- 平方根: \\sqrt{中身} 例: \\sqrt{2}
- 累乗: x^{2}
- 独立した数式: $$数式$$
- 文中の数式: $数式$

例: 実数 $x = \\frac{1}{2}$ について考える。
...
**良い例:**
問題: 実数 $x = \\frac{1}{\\sqrt{7}-2}$ の整数部分を $a$、小数部分を $b$ とする。

次の式を計算せよ: $$y = \\frac{x^{2} + 1}{x - 1}$$

**注意:**
- 複雑な数式は避け、シンプルに
- 数値は具体的に（例: $\\frac{1}{2}$ ではなく 0.5 でもOK）
- 読みやすさを最優先
...
【選択肢問題の場合】
{
  ...
  "correctAnswer": "正解の選択肢（choices配列のインデックス0-3）",
  "explanation": "解説"
}
...
JSON形式のみで回答し、他の説明は不要です。`;
  ...
  return { 
    type: 'text',
    question: '問題の生成に失敗しました。もう一度お試しください。', 
    expectedAnswer: '' 
  };
}
